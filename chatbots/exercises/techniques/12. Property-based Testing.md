...
```
    npm install --save-dev jest ts-jest @types/jest typescript fast-check
```
...
2. **Consent**
	- If `state.consentGiven === false`, no message must be sent.
		- In this exercise: `buildMessagePayload` should throw an error.
3. **Text handling**
...
4. **Length field correctness**
	- `payload.length` must equal the number of Unicode code points in `payload.text`
	  (not bytes, and not just `string.length`).
5. **Identifiers & retry**
